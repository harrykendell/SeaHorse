"""Optimisation scaffolding built around the Seahorse objective interface."""
...
class DCRABConfig:
    """Configuration for the Python-side dCRAB optimiser."""
...
@dataclass
class DCRABResult:
    """Summary of a completed dCRAB optimisation run."""
...
class DCRABOptimizer:
    """Python-side dressed CRAB optimiser using the native propagation objective."""
    ...
    def sample_coefficients(self) -> np.ndarray:
        """Sample a new coefficient vector from the active basis.

        Returns
        -------
        numpy.ndarray
            Float64 vector compatible with the active basis.
        """

        return self.basis.sample_coefficients(self.rng)

    def control_from_coefficients(self, coeffs) -> np.ndarray:
        """Convert basis coefficients into the full dressed control waveform.

        Parameters
        ----------
        coeffs:
            Coefficient vector for the current active basis.

        Returns
        -------
        numpy.ndarray
            Float64 control waveform including all accumulated dressed terms.
        """
    ...
    def evaluate_coefficients(self, coeffs) -> ControlEvaluation:
        """Build and evaluate the control generated by ``coeffs``.
        ...
        -------
        ControlEvaluation
            Objective evaluation of the generated control waveform.
        """
    ...
    def optimise(self) -> DCRABResult:
        """Run the dCRAB search until one of the stopping criteria is met.

        Returns
        -------
        DCRABResult
            Summary of the best control found, the optimisation history, and
            the number of iterations, propagations, and dressings completed.
        """
